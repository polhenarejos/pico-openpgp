    ...
    if (key_ref != EF_PIV_PIN && key_ref != EF_PIV_PUK)
    {
        ...
        if (meta[0] == PIV_ALGO_RSA1024 || meta[0] == PIV_ALGO_RSA2048 || meta[0] == PIV_ALGO_RSA3072 || meta[0] == PIV_ALGO_RSA4096 || meta[0] == PIV_ALGO_ECCP256 || meta[0] == PIV_ALGO_ECCP384)
        {
            ...
            if (meta[0] == PIV_ALGO_RSA1024 || meta[0] == PIV_ALGO_RSA2048 || meta[0] == PIV_ALGO_RSA3072 || meta[0] == PIV_ALGO_RSA4096)
            {
                ...
                put_uint16_t_be(mbedtls_mpi_size(&ctx.N), res_APDU + res_APDU_size);
                res_APDU_size += 2;
...
static int cmd_set_mgmkey()
{
    ...
    uint8_t touch_p2 = P2(apdu);
    if (touch_p2 != 0xFF && touch_p2 != 0xFE)
    {
        return SW_INCORRECT_P1P2();
    }
    ...
    uint8_t algo = apdu.data[0];
    uint8_t key_ref_tag = apdu.data[1];
    uint8_t key_len = apdu.data[2];

    if (apdu.nc != (3 + key_len))
    {
        return SW_WRONG_LENGTH();
    }

    if (key_ref_tag != EF_PIV_KEY_CARDMGM)
    {
        return SW_WRONG_DATA();
    }

    if (!((algo == PIV_ALGO_AES128 && key_len == 16) ||
          (algo == PIV_ALGO_AES192 && key_len == 24) ||
          (algo == PIV_ALGO_AES256 && key_len == 32) ||
          (algo == PIV_ALGO_3DES && key_len == 24)))
    {
        return SW_WRONG_DATA();
    }

    file_t *ef = search_by_fid(key_ref_tag, NULL, SPECIFY_ANY);
    if (!ef)
    {
        return SW_FILE_NOT_FOUND();
    }
    file_put_data(ef, apdu.data + 3, key_len);

    uint8_t *meta = NULL, new_meta[4];
    int meta_len = 0;
    if ((meta_len = meta_find(key_ref_tag, &meta)) <= 0)
    {
        return SW_REFERENCE_NOT_FOUND();
    }
    ...
    {
        touch_policy = TOUCHPOLICY_ALWAYS;
    }

    memcpy(new_meta, meta, 4);
    new_meta[0] = algo;
    new_meta[2] = touch_policy;
    meta_add(key_ref_tag, new_meta, sizeof(new_meta));
